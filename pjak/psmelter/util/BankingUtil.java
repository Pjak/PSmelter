 ...
 *
 * @author HelBorn Edited By Wyn
 *         ID's Fixed/Added: CW, Taverly, Player-Owned Port, Gnome NPC Bankers, Gnome Bank Booths, AlKarid Booths
 */
public class BankingUtil {
    ...
    public static boolean open() {
        ...
        if (!bank.isOnScreen() && (!Players.getLocal().isMoving() || Calculations.distance(Walking.getDestination(), ((Locatable) bank).getLocation()) > 4)) {
    ...
    public static void depositAllExcept(final int... ids) {
        ...
        for (int i = 0; i < 28; i++) {
            ...
            if (spot != null && checkID(spot.getChildId(), true, ids)) {
                deposit(spot.getChildId(), 0);
            }
        }
    }
 
    public static boolean checkID(final int id, boolean skip, final int... ids) {
        for (int curID : ids) {
            if (curID == id) {
                return !skip;
            }
    ...
    public static enum Tab {
        ...
        public WidgetChild getWidgetChild() {
            if (this != NONE && this != SEARCH && isOpen()) {
